function [ tf, frex ] = tfdecomp( data, MEEG, min_freq, max_freq, num_frex, spacing, method, varargin )
...
%       spacing         'log' (logarithmic) or 'lin' (linear) freq spacing
%       method          'means', 'trials', or 'phase'
%
% If METHOD is 'means' or 'trials', also pass the following:
%       times2save      Array of times at which to decompose
%       baseline        2x1 vector of baseline start and end time
%
...
%       frex                 The frequencies at which decomposition
%                               took place
%
...
if ~strcmpi(spacing, 'lin') && ~strcmpi(spacing, 'log')
    error(['Call to tfdecomp failed: ''' spacing ''' invalid value for input parameter ''spacing'' (try LOG or LIN)']);
end
